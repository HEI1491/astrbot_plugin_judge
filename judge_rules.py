...
META_FAST_REGEXES = (
    re.compile(r"把.*(需求|代码).*(贴|发|给|丢|贴我|发我)", re.IGNORECASE),
    re.compile(r"(把|将).*(代码|报错).*(发|贴|给).*(看看|我看看|我看下|我看一眼)", re.IGNORECASE),
    re.compile(r"(你要|想要|准备).*(写|搞).*(哪块|什么|哪个).*(编程|代码)", re.IGNORECASE),
    re.compile(r"(python|node|javascript|java).*(还是|或|或者).*(别的|其它|其他)", re.IGNORECASE),
)
