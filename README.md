...
智能分析用户意图 • 动态路由模型 • 降低API成本 • 提升响应速度
...
- **🧠 智能分流**：自动识别"弱智"问题与"烧脑"问题，分别路由至快速模型或高智商模型。
- **⚖️ 负载均衡**：支持配置多个高智商模型提供商，随机/轮询调用，避免单点故障。
- **🛡️ 规则兜底**：当判断模型不可用时，自动降级为关键词/正则匹配模式。
- **💰 预算控制**：内置每日/每月预算限制，防止Token超支。
...
配置文件位于 `data/plugins/astrbot_plugin_judge/config.json`。
